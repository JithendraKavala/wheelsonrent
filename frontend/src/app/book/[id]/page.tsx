...
export default function BookVehiclePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:${process.env.NEXT_PUBLIC_API_PORT}/rentals/book`, {
        method: 'POST',
        headers: {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "An error occurred while booking. Please try again later.",
        variant: "destructive",
        duration: 5000,
      });
    } finally {
